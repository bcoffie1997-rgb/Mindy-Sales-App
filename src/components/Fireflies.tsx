...
interface Transcript {
  id: string; title: string; meeting_date: string; duration: number
  transcript_url: string; participants: string[]; overview: string
  short_summary: string; action_items: string; keywords: string[]
  matched_lead_id: string | null; metadata: { attendees?: Attendee[] }
}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <Mic size={20} className="text-purple-400" /> Fireflies Notes
          </h2>
        ...
        <button onClick={runSync} disabled={syncing} className="btn-primary flex items-center gap-2 text-sm px-4 py-2 self-start sm:self-auto">
        ...
        <div className="card p-8 text-center space-y-3">
          ...
          <p className="text-slate-400">
            {transcripts.length === 0
              ? 'No transcripts yet. Click "Sync from Fireflies" to import your calls.'
          ...
          {filtered.map(t => {
            ...
            return (
              <div key={t.id} className="card overflow-hidden">
                <div
                  className="flex items-start gap-3 sm:gap-4 p-4 cursor-pointer hover:bg-white/[0.02] transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {t.matched_lead_id && <span className="badge-purple text-xs">Lead matched</span>}
                        {t.transcript_url && (
                          <a href={t.transcript_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="text-slate-400 hover:text-white transition-colors">
                            <ExternalLink size={13} />
                          </a>
                        )}
                        {hasDetails && <span className="text-slate-500">{isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 flex-wrap">
                      {t.duration && <span className="flex items-center gap-1"><Clock size={11} /> {dur(t.duration)}</span>}
                ...
                {isOpen && hasDetails && (
                  <div className="border-t border-white/10 p-4 space-y-4 bg-white/[0.02]">
                    ...
                    {t.keywords?.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1"><Tag size={11} /> Keywords</p>
                        <div className="flex flex-wrap gap-1.5">
                          {t.keywords.map((kw, i) => (
                            <span key={i} className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-xs text-slate-300">{kw}</span>
                          ))}
                        </div>
                      </div>
                    )}
